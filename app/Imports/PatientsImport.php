    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            try {
                ...
                // Check if vaccine already exists for this patient and type
                ...
                if ($existingVaccine) {
                    ...
                    if ($existingVaccine->tanggal_vaksin_pertama->format('Y-m-d') !== $tanggalVaksin->format('Y-m-d')) {
                        ...
                        Log::info("Row {$this->rowNumber}: Vaccine already exists with same date, skipping", [
                            'patient_id' => $patient->id,
                            'vaccine_id' => $existingVaccine->id,
                            'vaccine_type' => $vaccineType->nama_vaksin,
                        ]);
                ...
                Log::error("Row {$this->rowNumber}: Import error", [
